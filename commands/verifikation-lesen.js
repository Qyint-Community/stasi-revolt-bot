...
module.exports = {
  name: "bewerbungen",
  description: "Durchsuche die Bewerbungen!",
  ...
  execute(message, args) {
    ...
    if (!fs.existsSync(userDir)) {
      return message.reply(`:file_folder::mag_right: Keine Bewerbungen offen.`);
    }
    ...
    if (!fileName) {
      fs.readdir(userDir, (err, files) => {
        ...
        message.channel.sendMessage(`### :open_file_folder: Bewerbungen:\n${fileList}`, { split: true });
      });
    } else {
      const filePath = path.join(userDir, `${fileName}.txt`);

      if (!fs.existsSync(filePath)) {
        return message.reply(`:open_file_folder::mag_right: Bewerbung \`${fileName}\` existiert nicht.`);
      }
      ...
      fs.readFile(filePath, 'utf8', (err, fileContents) => {
        ...
        message.channel.sendMessage(`### :open_file_folder: Bewerbung \`${fileName}\`:\n\`\`\`${fileContents}`);
      });
    }
  },
};
